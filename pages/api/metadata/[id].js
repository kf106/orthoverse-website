...
function hexToLat(input) {
  ...
  result =
    result +
    Math.floor((parseInt(input.slice(1, 4), 16) * 90) / 4095) +
    "." +
    parseInt(input.slice(4, 10), 16);
  return result;
}
...
function hexToLong(input) {
  ...
  result =
    result +
    Math.floor((parseInt(input.slice(1, 4), 16) * 180) / 4095) +
    "." +
    parseInt(input.slice(4, 10), 16);
  return result;
}
...
export default function handler(req, res) {
  ...
  if (id == "contract.json") {
    const contract = {
      ...
      description:
        "The Orthoverse is the largest NFT collection in existence, with over 1.4 quindecillion tokens in existence. Every Ethereum address already has an NFT representing a parcel of land in the Orthoverse. Visit our site to reveal your NFT and to learn more about the project.",
    ...
    if (shortId.length > 40 || parseInt(castle) > 15) {
      ...
      const metadata = {
        ...
        image:
          "https://orthoverse.io/api/img/" + shortId + "-" + castle + ".png",
      };
